...
local UseRage = false
local spinSpeed = math.rad(200)
local isSpinning = false
local character = game.Players.LocalPlayer.Character
local primaryPart = character and character:FindFirstChild("HumanoidRootPart")

local function spinCharacter()
    while isSpinning and primaryPart do
        primaryPart.CFrame = primaryPart.CFrame * CFrame.Angles(0, spinSpeed, 0)
        wait(0.1)
    end
end
...
local Window = Rayfield:CreateWindow({
   Name = "PRIVATE Blade Ball By Lord Hippo :3",
   LoadingTitle = "PRIVATE SCRIPT BY LORDHIPPO",
   LoadingSubtitle = "PRIVATE BY LORDHIPPO",
   ConfigurationSaving = {
      Enabled = false,
      FolderName = "PRIVATE Hippo",
      FileName =  "PRIVATE"
   },
   Discord = {
      Enabled = true,
   ...
   KeySystem = false,
   KeySettings = {
      Title = "PRIVATE KEY SYSTEM",
      Subtitle = "Key System",
      Note = "Join the discord (discord.gg/Noserver)",
      FileName = "Pippo",
      SaveKey = false,
...
local DiscordInviteButton = AutoParry:CreateButton({
    Name = "Copy Discord Invite",
    Callback = function()
        local inviteLink = "https://discord.gg/drZw2qft2A"
        if setclipboard then
            setclipboard(inviteLink)
            notify("Discord Invite", "Invite link copied to clipboard!", 1)
        else
            print("setclipboard is not available in this environment.")
        end
    end
})

local ToggleSpinButton = AutoParry:CreateButton({
    Name = "Toggle Character Spin",
    Callback = function()
        toggleSpin()
    end,
})
